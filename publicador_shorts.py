...
YT_TOKEN_JSON = os.environ.get("YOUTUBE_TOKEN_FR_SHORTS")
HORARIO_ALVO = os.environ.get("HORARIO_ALVO")

print(f"🚀 DÉMARRAGE MATRIX SERVER SHORTS (FR) POUR: {HORARIO_ALVO}")
...
aba_shorts = gc.open_by_key("1KgIjWrLUVlllhlZB1R9fkHGxxZlLsax1aOVGZrYwgnU").worksheet("FR_SHORTS")
...
PASTA_TEMP = "/tmp/fabrica_shorts_fr"
...
ID_PLAYLIST_SHORTS_FR = "PLYfOuJNcmFM0"
...
def baixar_arquivo(file_id, destino):
    for tentativa in range(4):
        try:
            ...
            print(f"   ⚠️ Google Drive échoué. Nouvelle tentative... ({tentativa+1}/4)")
            time.sleep(5)
...
for index, linha in enumerate(dados, start=2):
    if str(linha.get('Status', '')).strip() == 'Ready for Audio' and str(linha.get('Horario', '')).strip() == HORARIO_ALVO:
        ...
        print(f"🎬 DÉMARRAGE SHORT: Ligne {index} - {persona} à {horario_str}")

        voz_escolhida = "fr-FR-DeniseNeural"

        print("   📥 Téléchargement images verticales (Maria Universal)...")
        ...
        print(f"   🎙️ Génération voix neurale et sous-titres ({voz_escolhida})...")
        ...
        print("   ✂️ Découpage du silence final pour la boucle...")
        ...
        print("   🎞️ Assemblage blocs visuels verticaux (1080x1920)...")
        ...
        print("   🔥 Mixage audio et finalisation du Short...")
        ...
        texto_convite = f"\n\n🔴 EN DIRECT — 24H AVEC NOTRE-DAME 🙏 Rejoignez-nous: https://youtube.com/channel/UC7dZrYzY22dO-h6RfRN1bsw/live\n\nPour la prière complète et profonde, visitez notre chaîne. Nous publions des prières puissantes chaque jour.\n\nNotre playlist: https://www.youtube.com/playlist?list=PLCKFHOkKiqGg"

        try:
            agora_paris = datetime.datetime.now(pytz.timezone('Europe/Paris'))
            data_hora_alvo = pytz.timezone('Europe/Paris').localize(datetime.datetime.strptime(f"{data_str} {horario_str}", "%Y-%m-%d %H:%M"))
            publish_at = data_hora_alvo.isoformat() if data_hora_alvo > agora_paris else None
        except: publish_at = None

        body = {
            "snippet": {"title": titulo[:100], "description": f"{descricao_ia}{texto_convite}", "tags": tags_lista, "categoryId": "22", "defaultLanguage": "fr-FR", "defaultAudioLanguage": "fr-FR"},
        ...
        for tentativa in range(3):
            try:
                ...
                print(f"   🎉 SUCCÈS! Short {video_id} publié.")
                try:
                ...
                try:
                    if ID_PLAYLIST_SHORTS_FR and not ID_PLAYLIST_SHORTS_FR.startswith("PLACEHOLDER"): youtube.playlistItems().insert(part="snippet", body={"snippet": {"playlistId": ID_PLAYLIST_SHORTS_FR, "resourceId": {"kind": "youtube#video", "videoId": video_id}}}).execute()
                ...
                except Exception as e: print(f"   ⚠️ Échec de mise à jour: {e}")
                break
            except Exception as e:
                print(f"   ❌ Erreur YouTube (Tentative {tentativa+1}/3): {e}")
                time.sleep(15)

print("\n🚀 MATRIX SERVER SHORTS FR ARRÊTÉ.")
