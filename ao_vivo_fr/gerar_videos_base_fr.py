#!/usr/bin/env python3
"""
gerar_videos_base_fr.py — GitHub Actions: creates slideshow videos for videos_base/ of live FR
...
Environment variables:
  GOOGLE_CREDENTIALS_FR  — service account JSON
  ...
  OUTPUT_DIR             — output directory (default: videos_base_fr)
"""
...
TARGET_W        = 1280      # 720p
TARGET_H        = 720
DIR_SAIDA       = Path(os.environ.get("OUTPUT_DIR") or "videos_base_fr")
...
def get_drive():
    raw  = os.environ.get("GOOGLE_CREDENTIALS_FR", "")
    if not raw:
...
def main():
    print("=" * 60)
    print(f"gerar_videos_base_fr.py — {VIDEO_PREFIX.upper()} — {NUM_VIDEOS} videos")
